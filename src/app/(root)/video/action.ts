...
export async function getVideo(id: number): Promise<VideoType | undefined> {
    ...
    return await db.select({ id: Video.id, status: Video.status, style: Video.style, duration: Video.duration, storyboard: Video.storyboard, audioUris: Video.audio_uri, imageUris: Video.image_uri, captions: Video.captions, createdAt: Video.created_at })
        .from(Video)
...
export async function deleteVideo(id: number): Promise<void | number> {
    const [ { deletedId, folder } ]: { deletedId: number, folder: string | null }[] = await db.delete(Video).where(eq(Video.id, id)).returning({ deletedId: Video.id, folder: Video.folder });
